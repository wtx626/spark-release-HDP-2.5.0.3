...
/**
  ...
  * ②TaskScheduler
  * ③SchedulerBacken
  * 注册应用程序给master，如果没有正在运行的SparkContext的话就给其分配资源
  **/
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {

  //用户代码调用SparkContext中的堆栈信息方法中
  //功能描述：获取当前SparkContext的当前调用栈，将栈里最靠近栈底的属于Spark核心的类压入callStack的栈顶，
  ...
  // 这里是Spark消息系统的具体实现，后台会自动开启一个线程来监听事件，现在基本上所有的分布式系统都是通过消息驱动的方式来进行并发执行的
  ...
  try {
    ...
    _eventLogDir =
      if (isEventLogEnabled) {
        //因为如果不配置的话，它的默认的路径就是这个EventLoggingListener.DEFAULT_LOG_DIR＝"/tmp/spark-events"
    ...
    //在listenerBus接收到相应的event之后，会遍历所有的listener来处理相应的事件
    ...
    SparkEnv.set(_env)
    //1.元数据清理器，这里清理的元数据指的就是之前进行persist操作的数据，更具体点说就是persistentRdds中的数据
    ...
    _ui.foreach(_.bind())
    //5.这里面就是配置hadoop系统的信息，其中有一个比较重要的参数io.file.buffer.size，io.file.buffer.size都被用来设置缓存的大小。
    // 不论是对硬盘或者是网络操作来讲，较大的缓存都可 以提供更高的数据传输，但这也就意味着更大的内存消耗和延迟。
    // 这个参数要设置为系统页面大小的倍数，以byte为单位，默认值是4KB，一般情况下，可以设置为64KB（65536byte）。
    _hadoopConfiguration = SparkHadoopUtil.get.newConfiguration(_conf)
    // 6. 将spark.jars和spark.files中的文件添加到env.rpcEnv.fileServer上
    ...
    _taskScheduler = ts
    //DAGScheduler在当前的资源分配阶段是没什么用的，它是用于在构建job的阶段的
    ...
    //Stadalone模式下SparkDeploySchedulerBackend的start方法会在app注册成功之前阻塞进程
    //给各个需要appId的组件或变量指定其appId
    _applicationId = _taskScheduler.applicationId()
    _applicationAttemptId = taskScheduler.applicationAttemptId()
    _conf.set("spark.app.id", _applicationId)
    _ui.foreach(_.setAppId(_applicationId))
    //另外关于spark初始化还有这一步比较重要的操作，就是blockManager的初始化。
    ...
    // 12. 后台后启动一个线程，一直在执行clean数据的操作，
    // 包括RDD，shuffle，广播变量，累加器，checkpoint的数据
    _cleaner =
