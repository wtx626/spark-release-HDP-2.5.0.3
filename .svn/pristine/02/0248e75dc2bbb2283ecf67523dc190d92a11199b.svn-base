...
private[spark] class Client(
  ...
  def createApplicationSubmissionContext(
    ...
    sparkConf.getOption(CONF_SPARK_YARN_APPLICATION_TAGS).map(StringUtils.getTrimmedStringCollection(_)).filter(!_.isEmpty()).foreach {
      tagCollection =>
        try {
          ...
          // doesn't support it.
          val method = appContext.getClass().getMethod("setApplicationTags", classOf[java.util.Set[String]])
  ...
  private def createContainerLaunchContext(newAppResponse: GetNewApplicationResponse)
    ...
    // send the acl settings incto YARN to control who has acess via YARN interfaces
