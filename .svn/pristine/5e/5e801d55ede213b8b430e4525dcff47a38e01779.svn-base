...
object SparkEnv extends Logging {
  ...
  private def create(
    ...
    // 6. 在RpcEnv中新建或设置endpoint，mapOutputTracker会使用这个方法
    def registerOrLookupEndpoint(
                                  name: String, endpointCreator: => RpcEndpoint):
    RpcEndpointRef = {
      if (isDriver) {//Driver上，新建endpoint
        logInfo("Registering " + name)
        rpcEnv.setupEndpoint(name, endpointCreator)
      } else {//executor上，设置driver的ref
    ...
    // 10. 是否使用以前的memoryManage管理机制，这个UnifiedMemoryManager是spark 1.6.＊之后最后要的改进
