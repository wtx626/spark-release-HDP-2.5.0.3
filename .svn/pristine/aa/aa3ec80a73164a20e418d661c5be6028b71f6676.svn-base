...
private[spark] class HashShuffleWriter[K, V](
  ...
  override def write(records: Iterator[Product2[K, V]]): Unit = {
    val iter = if (dep.aggregator.isDefined) {
      if (dep.mapSideCombine) {//判断是否需要做聚合，若是，则聚合为records
    ...
    for (elem <- iter) {
      val bucketId = dep.partitioner.getPartition(elem._1)//获得该element需要写入的partition
      shuffle.writers(bucketId).write(elem._1, elem._2)//实际调用org.apache.spark.shuffle.FileShuffleBlockResolver.forMapTask进行写入
    }
  }
