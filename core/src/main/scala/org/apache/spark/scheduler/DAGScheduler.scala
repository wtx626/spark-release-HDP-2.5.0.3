...
class DAGScheduler(
  ...
  private def getShuffleMapStage(
    ...
    shuffleToMapStage.get(shuffleDep.shuffleId) match {
      case Some(stage) => stage//if stage is created,directly return stage
      case None =>
        // We are going to register ancestor shuffle dependencies
        //判断该stage的父stage是否已经生成，如果没有，则生成它们
  ...
  private def newOrUsedShuffleStage(
    ...
    if (mapOutputTracker.containsShuffle(shuffleDep.shuffleId)) {
      //stage已经被计算过了则直接从mapOutPutTracker中获取计算结果
   ...
   */
  /**
    * 里面先确定patition的数量是正常范围内， 然后创建JobId， 如果partions是0 代表最终没有task， 所以直接返回JobWaiter，
    * 如果定partition大于0， 则创建JobWaiter用来返回去执行 awaitResult， 然后通过eventProcessLoop 把JobSubmitted的event加入进去
    * */
  def submitJob[T, U](
      rdd: RDD[T],
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    val tasks: Seq[Task[_]] = try {
      stage match {
          //generate a task for every partition ,usually a partition is distributed by a core
        case stage: ShuffleMapStage =>
          partitionsToCompute.map { id =>
    ...
    if (tasks.size > 0) {
      //ShuffleMapStage:Submitting 2 missing tasks from ShuffleMapStage 0 (MapPartitionsRDD[3] at map at HelloWorld.scala:29)
