  ...
  extends TaskScheduler with Logging
{
  ...
  def initialize(backend: SchedulerBackend) {
    ...
    //默认是FIFO，so buildPools啥也没干
    schedulableBuilder.buildPools()
  }

  def newTaskId(): Long = nextTaskId.getAndIncrement()
  //启动这个被绑定的SparkDeploySchedulerBackend（standalone模式下）
  override def start() {
  ...
  override def submitTasks(taskSet: TaskSet) {
    ...
    //ShuffleMapStage:Adding task set 0.0 with 2 tasks
    logInfo("Adding task set " + taskSet.id + " with " + tasks.length + " tasks")
    this.synchronized {
      //TaskSetManager will allocate computing resource using locality aware by data partition
      ...
      //scheduler strategy FIFO or FAIR
