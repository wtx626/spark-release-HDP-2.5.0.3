  ...
  with Serializable
{
  ...
  def combineByKeyWithClassTag[C](
      createCombiner: V => C,
      mergeValue: (C, V) => C,//func
      mergeCombiners: (C, C) => C,//func
   ...
   */
  /**
    * 这个里面其实主要就是设置了hadoopconf的属性，然后设置到了FileOutputFormat里面
    * */
  def saveAsHadoopFile(
      path: String,
      keyClass: Class[_],
   ...
   */
  /**
    * 1创建了一个writer： val writer = new SparkHadoopWriter(hadoopConf)
    * 2创建了writeToFile这个function， 这个function会被作为一个Job提交： self.context.runJob(self, writeToFile)
    * */
