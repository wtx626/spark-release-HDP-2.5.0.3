...
object SparkSubmit {
  ...
  private def submit(args: SparkSubmitArguments): Unit = {
    ...
    printStream.print("submit子类参数为：childArgs\t"+childArgs+"\tchildClasspath:"+childClasspath+"\tsysProps:"+sysProps+"\tchildMainClass:"+childMainClass)
  ...
  private def runMain(
    ...
    /**
      ...
      * (实验性)当在driver中加载类时，是否用户添加的jar比Spark自己的jar优先级高。
      * 这个属性可以降低Spark依赖和用户依赖的冲突。它现在还是一个实验性的特征。
      * */
    val loader =
